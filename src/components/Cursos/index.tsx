...
const Cursos = ({ galleryCurso }: CursoProps) => {
  ...
  const initScrollAnimations = useCallback(() => {
    gsap
      ...
      .from(items.current!, { opacity: 0 })
      .from(items1.current!, { opacity: 0 })
      .from(items2.current!, { opacity: 0 })
      .from(items3.current!, { opacity: 0 })
      .from(items4.current!, { opacity: 0 })
      .from(items5.current!, { opacity: 0 })
      .from(items6.current!, { opacity: 0 })
      .from(items7.current!, { opacity: 0 })
      .from(items8.current!, { opacity: 0 })
      .from(items9.current!, { opacity: 0 })
      .from(items10.current!, { opacity: 0 })
  }, [])
  useEffect(() => {
